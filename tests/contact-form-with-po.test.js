...
import ContactPageObject from '../pages/contactPageObject'
...
test('I can validate errors on the "Contact" page', async t => {
    ...
    await t
        .expect(ContactPageObject.forenameErrorSelector.textContent).eql('Forename is required')
        .expect(ContactPageObject.emailErrorSelector.textContent).eql('Email is required')
        .expect(ContactPageObject.textAreaErrorSelector.textContent).eql('Message is required')
})
...
test('I can populate all mandatory fields and successfully submit the "Contact" page form', async t => {
    ...
    await ContactPageObject.inputTextArea('Lorem Ipsum is simply dummy text of the printing and typesetting industry.');
    await ContactPageObject.submitForm();
